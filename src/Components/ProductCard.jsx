...
const ProductCard = ({product}) => {
  ...
  return (
    <div className='w-[250px]  h-[350px] rounded-xl bg-[#FAFAFA] p-[20px] hover:border hover:border-secondary hover:transform hover:scale-105 transition-all ease-in-out duration-300'>
      ...
      <button onClick={()=>addToCart(product)} className='flex items-center justify-center mb-3 w-full py-1 bg-secondary text-white cursor-pointer'>
        <ShoppingCartIcon />
      </button>
      <hr className='w-full' />
      <div>
       ...
       <div className='flex items-center gap-4'>
        <p className='text-base font-normal line-through text-gray-400'>{currency}{product.price}</p>
           <p className='text-base font-normal '>{currency}{product.offerPrice}</p>
       </div>
    </div>
  )
}
