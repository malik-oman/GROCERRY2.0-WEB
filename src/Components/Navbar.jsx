        ...
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
           ...
           <Link>
            ...
            <div className="hidden sm:flex items-center gap-8">
             ...
             <button onClick={()=>navigate('/admin')} className="bg-primary text-white px-6 py-2 cursor-pointer rounded-full ">Admin Dashboard</button>

                <div onClick={()=>navigate('/cart')} className="relative cursor-pointer">
                   <ShoppingBag className="w-5 h-5"/>
                    <button className="absolute -top-2 -right-3 text-xs text-white  w-[18px] h-[18px] rounded-full bg-primary">{cart?cart.length : 0}</button>
                </div>

                
                <div onClick={()=>navigate('/wishlist')} className="relative cursor-pointer">
                   <Heart className="w-5 h-5"/>
                    <button className="absolute -top-2 -right-3 text-xs text-white  w-[18px] h-[18px] rounded-full bg-primary">{favorite?favorite.length : 0}</button>
                </div>
                ...
                user?(
                    <div className="relative group">
                        ...
                        <ul className="text-white">
                           <p onClick={()=>navigate('/my-orders')} className="cursor-pointer hover:bg-primary py-1 px-3">My Orders</p>

                                  <p onClick={logout} className="cursor-pointer hover:bg-primary py-1 px-3">Logout</p>

                        </ul>
                        </div>
                    </div>
                ):(
                        <button onClick={()=>navigate('/login')} className="cursor-pointer px-8 py-2 bg-primary  hover:bg-secondary transition text-white rounded-full">
                    Login
                </button>
                )
            }
            ...
            <div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden z-50`}>
               <Link onClick={()=> setOpen(false)} to={'/'} className={isActive('/')} >Home</Link>
                ...
                <Link onClick={()=> setOpen(false)} to={'/contact'} className={isActive('/contact')} >Contact</Link>
                 ...
                 <button onClick={()=>navigate('/admin')} className="bg-primary text-white px-4 py-2 cursor-pointer rounded-full ">Admin Dashboard</button>
