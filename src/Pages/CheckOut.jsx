    ...
    <div className='py-12 bg-[#0B482F]' style={{backgroundImage: `url(${assets.footer_img})`}}>
      ...
      <div className='flex flex-col md:flex-row items-center justify-evenly gap-6'>
        ...
        <div>
          ...
          <div className='my-5 max-w-4xl w-full p-4 rounded-md border bg-white border-primary'>

            {
             ...
             <p>Total:{currency}{getCartTotal}</p>
          </div>
        </div>
        ...
        <div className='text-white flex flex-col gap-6'>
         ...
         <select className='w-full outline-none border border-primary p-2' value={address} onChange={(e)=>setAddress(e.target.value)} >
          <option className='text-gray-800'>MULTAN</option>
          <option className='text-gray-800'>MUZAFFARGARH</option>
         </select>

         <button onClick={()=>navigate('/add-address')} className='bg-primary text-white cursor-pointer px-6 py-2'>Add New Address</button>

          </div>

            <div className='flex flex-col gap-6'>
         <label htmlFor="address">Select Payment Method</label>
         <select className='w-full outline-none border border-primary p-2' value={paymentMethod} onChange={(e)=>setPaymentMethod(e.target.value)} >
          <option value={"cod"} className='text-gray-800'>COD</option>
          <option value={"online"} className='text-gray-800'>Online</option>
         </select>
         </div>
      
           <button className='bg-primary text-white cursor-pointer px-6 py-2'>{paymentMethod === 'cod' ? 'Place Order' : 'Pay Now'}</button>
          </div>
          </div>
         </div>
